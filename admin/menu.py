...
app_id = 'wx05429a8ee93be82d'
app_secret = '5012ec05c9c049bcfed217767e635efa'
...
def create_menu(access_token, filename='menu.json'):
    ...
    with open(filename, 'r') as f:
        data = json.load(f)
    # post to menu_url
    menu_url = 'https://api.weixin.qq.com/cgi-bin/menu/create?access_token={access_token}'.format(
...
def get_menu(access_token):
    ...
    :rtype: json
    """
    menu_url = 'https://api.weixin.qq.com/cgi-bin/menu/get?access_token={access_token}'.format(
        access_token=access_token)
...
def delete_menu(access_token):
    ...
    :rtype: json
    """
    menu_url = 'https://api.weixin.qq.com/cgi-bin/menu/delete?access_token={access_token}'.format(
        access_token=access_token)
    r = requests.get(menu_url)
    assert r.status_code == 200
    return r.json()
